...
feature -- Chunking
        ...
        do
            ...
            across iterable as ic loop
                ...
                if l_current_chunk.is_empty then
                    ...
                    if attached l_previous_value as prev and then not prev.is_equal (l_current_value) then
    ...
    chunk_every (a_count: INTEGER; a_step: INTEGER; a_leftover: detachable ITERABLE[G]): ENUM [ENUM[G]]
        ...
        do
            ...
            if attached a_leftover then
                create l_leftover_enum.make (a_leftover)
            ...
            across iterable as ic loop
                ...
                if l_current_chunk.count = a_count or else l_position \\ a_step = 0 then
                    if l_current_chunk.count = a_count then
                        l_result.extend (create {ENUM[G]}.make (l_current_chunk))
                        create l_current_chunk.make (a_count)
                    end
            ...
            -- Handle last chunk if not empty
            if not l_current_chunk.is_empty then
                -- Try to fill incomplete chunk with leftover elements
                if l_current_chunk.count < a_count and then attached l_leftover_cursor as lc then
                    from
                    until
                        l_current_chunk.count = a_count or lc.after
                    loop
                        l_current_chunk.extend (lc.item)
                        lc.forth
                    end
                end
    ...
    chunk_every_discard (a_count: INTEGER; a_step: INTEGER): ENUM [ENUM[G]]
        ...
        do
            ...
            -- Only add last chunk if it's complete
            if l_current_chunk.count = a_count then
                l_result.extend (create {ENUM[G]}.make (l_current_chunk))
            end
